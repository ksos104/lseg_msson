...
class LSegmentationModuleZS(pl.LightningModule):
    def __init__(self, data_path, dataset, batch_size, base_lr, max_epochs, **kwargs):
        ...
        ## text_same_diff
        # self.cross_entropy_loss = nn.CrossEntropyLoss(weight=torch.tensor([1, 1, 0.5, 0.5]))
    ...
    def forward(self, x, class_info, target=None):
        return self.net(x, class_info, target)
    ...
    def training_step(self, batch, batch_nb):
        if self.args.finetune_mode:
            ...
            if self.args.nshot != 0:
                ...
                class_info = batch['class_id']
            with amp.autocast(enabled=self.enabled):
                out, proxy_loss, proto_loss = self(img, class_info, target)
                
                # if self.net.use_proxy:
                #     out[:,-self.net.n_proxy:,...] = out[:,-self.net.n_proxy:,...] * target.unsqueeze(1)
                
                loss = self.criterion(out, target) + proxy_loss + proto_loss
                loss = self.scaler.scale(loss)
    ...
    def validation_step(self, batch, batch_nb):
        if self.args.finetune_mode and self.args.nshot == 5:
            ...
            class_info = batch['class_id']
            out, proxy_loss, proto_loss = self(img, class_info)

            val_loss = self.criterion(out, target) + proxy_loss + proto_loss
            # 3. Evaluate prediction
            if self.args.benchmark == 'pascal' and batch['query_ignore_idx'] is not None:
    ...
    def validation_epoch_end(self, outs):
        ...
        self.val_miou_list.append(val_miou)
        if self.current_epoch > 2:
            if (self.val_miou_list[-1] - self.val_miou_list[-2] < 0) and (self.val_miou_list[-2] - self.val_miou_list[-3] < 0):
                print('End this loop!')
                exit()
    ...
    def configure_optimizers(self):
        ...
        '''
        params_list.append(
            {"params": self.net.proxy, "lr": self.base_lr * 10}
        )
        ...
        if self.other_kwargs["midasproto"]:
            ...
            opt = torch.optim.SGD(
                params_list,
                lr=self.base_lr,
                momentum=self.other_kwargs['momentum'],
                weight_decay=self.other_kwargs["weight_decay"],
            )
    ...
    def criterion(self, logit_mask, gt_mask):
        ...
        n_label = logit_mask.size(1)
        logit_mask = logit_mask.view(bsz, n_label, -1)
