...
class DINODecoder(nn.Module):
    ...
    def forward_features(self, features, attentions):
        ...
        bs = features.shape[0]
        hw = int(math.sqrt(features.shape[1]))
        features = features.permute(0,2,1).reshape(bs, self.conv_dim, hw, hw)
        attentions = attentions.reshape(bs, self.n_heads_list[0], hw, hw)
        ...
        features = features.reshape(bs, self.conv_dim, hw*hw).permute(0,2,1)
        attentions = attentions.reshape(bs, self.n_heads_list[-1], hw*hw)
